...
- **渐变过渡**（可选）：每个锚点可设过渡时长（0=立即切换），过渡带内亮度/色温按线性插值从上一时段参数渐变到目标参数；步进间隔可调（默认 5 分钟）；带内开灯直接应用当前插值；过渡步进只调已亮的灯，绝不主动开灯
...
- 过渡步进定时器按"渐变步进间隔"（默认 5 分钟）推进，沿用状态机：容差内跳过、**绝不主动开灯**
- 过渡带内开灯，直接应用该时刻的插值参数；手动调整会在下一次步进被拉回轨道
